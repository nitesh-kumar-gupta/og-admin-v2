...
export class PlanService extends BaseService {
  ...
  getIntialPlanData(){
    return this._http.get(`${this._url}/plans/pfeature/initialPlanData`).pipe(
      map(this.extractData),
      catchError(this.handleError)
    )
  }
   fetchPlanFeatures(plan){
     return this._http.get(`${this._url}/plans/${plan}/planFeatures`).pipe(
      map(this.extractData),
      catchError(this.handleError)
    )
  }
   updatePlan(data){
    return this._http.put(`${this._url}/plan/updatePlan`,data).pipe(
      map(this.extractData),
      catchError(this.handleError)
    )
  }    
