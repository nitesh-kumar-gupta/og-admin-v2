...
import { CurrentCompany } from '../models/company.model';
...
export class SubDomainService extends BaseService{
  ...
  setCurrentCompany(company: any){
    let cookie = CookiesService.readCookie('storage');
  ...
  subDomainExists():Promise<any> {
    if (this.subDomain.is_sub_domain_url ) {
      var promise = this._adminService.isSubDomainExist(this.subDomain.sub_domain).toPromise()
      promise.then((result: any) => {
          ...
          .catch((err) => {
            if (err.error.code === 'E_COMPANY_NOT_FOUND') {
              ...
              if(CookiesService.readCookie('storage')){
                let sub_domain = CookiesService.readCookie('currentCompany');
                if(sub_domain === this.subDomain.sub_domain){
                  CookiesService.createCookie('storage','',-1);
                }else{
                  let storage= CookiesService.readCookie('storage');
  ...
  checkCompanyMembership(){
    this._adminService.isCompanyMember(this.subDomain.company_id, this.subDomain.user_id)
      .subscribe(
        (data: any) => {},
        (response: any) => {
